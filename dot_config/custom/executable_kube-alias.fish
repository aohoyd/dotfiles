...
abbr -a -g kgev kubectl get events --sort-by='.lastTimestamp'
abbr -a -g ks kubeswitch
abbr -a -g kns kubeswitch ns
abbr -a -g ked kubectl edit

abbr -a -g kki kubectl konfig import $_kube_konfig_files

function kkm -w "kubectl konfig merge"
    for i in $_kube_konfig_files 
        set -l filename (echo $i | sed 's@.*/@@; s@\..*@@')
        yq -i ". |= (.contexts[0] |= (.context.cluster = \"$filename\", .context.user = \"$filename\", .name = \"$filename\"), .users[0].name = \"$filename\", .clusters[0].name = \"$filename\")" $i
    end
    kubectl konfig merge ~/.kube/config $_kube_konfig_files > ~/.kube/config
end
